---
title: "Google Advent of Agents S2 Day 3：Flash-Lite、成本与任务分层"
date: 2026-05-07
category: academy
description: "不要把最强模型用于所有步骤，而是按任务风险选择模型。"
plainSummary: "Season 2 Day 3 的原创工程讲义：从当天主题提炼设计边界、实践任务、生产风险和复核清单。"
difficulty: advanced
...
![Google Advent of Agents S2 Day 3：Flash-Lite、成本与任务分层 封面](/images/academy/google-advent-of-agents/season-2/day-03-flash-lite-cost-task-layering/cover.svg)

**说明：** 本文以 Google Advent of Agents Season 2 Day 3 的公开主题为主线，结合 Season 1 的相关主题做补充。它不是官方译文，也不复述原文，而是改写成本站的 Agent 工程学习笔记。

## 这一天要解决什么

不要把最强模型用于所有步骤，而是按任务风险选择模型。

如果把这一天只当成教程链接，很容易读完就过去了。更好的读法是把它变成一个工程问题：**我的 Agent 系统今天要多一个什么可审查能力？**

今天的目标产物是：一张模型路由表：哪些步骤用 Pro，哪些步骤用 Flash-Lite，哪些必须人工审批。

## 学习流程图

![Day 03 学习流程图](/images/academy/google-advent-of-agents/season-2/day-03-flash-lite-cost-task-layering/diagram.svg)
...
- 模型选择是系统设计，不是价格表选择。
- 低风险、可验证任务适合低成本模型。
- 高风险动作仍需要强模型、eval 或人类审批。

这三个概念可以作为读文章、写代码和做复盘时的抓手。只要其中一个说不清，系统通常还停留在 demo 阶段。

## 工程设计方式

把 Day 3 放进真实项目时，建议用下面这张小表约束设计：
...
一个客服质检 Agent 可以把任务分成三层：Flash-Lite 做标签、摘要和格式检查；Pro 做争议判断和复杂推理；涉及退款、赔付或账户变更时交给人工审批。这样不是单纯省钱，而是让每种推理强度对应清楚的风险等级。
...
- **业务触发：** 客服、审核、内容整理这类高频任务会迅速放大模型成本。
- **Agent 边界：** 低风险分类和格式化走轻量模型，复杂判断、争议处理和不可逆动作升级。
- **验收证据：** 路由日志能显示每次任务为什么选择某个模型，并能回放成本和质量差异。

## 最小 Lab

把一个 Agent 的摘要、分类、格式校验步骤拆成低成本模型路径，只把高风险推理留给强模型。
...
Season 1 Day 17 / Day 10：模型选择、上下文压缩和成本意识。

Season 1 更像“从零到生产”的基础路线，Season 2 更像“把系统加厚”。读 Day 3 时，不需要回头逐日翻 Season 1，但应该知道它在补哪一层背景。
