---
title: "Google Advent of Agents S2 Day 2：用 ADK 搭出可维护 Agent"
date: 2026-05-07
category: academy
description: "把第一个 ADK Agent 写成项目骨架，而不是一次性 demo。"
plainSummary: "Season 2 Day 2 的原创工程讲义：从当天主题提炼设计边界、实践任务、生产风险和复核清单。"
difficulty: advanced
...
![Google Advent of Agents S2 Day 2：用 ADK 搭出可维护 Agent 封面](/images/academy/google-advent-of-agents/season-2/day-02-adk-agents-gemini-pro/cover.svg)

**说明：** 本文以 Google Advent of Agents Season 2 Day 2 的公开主题为主线，结合 Season 1 的相关主题做补充。它不是官方译文，也不复述原文，而是改写成本站的 Agent 工程学习笔记。

## 这一天要解决什么

把第一个 ADK Agent 写成项目骨架，而不是一次性 demo。

如果把这一天只当成教程链接，很容易读完就过去了。更好的读法是把它变成一个工程问题：**我的 Agent 系统今天要多一个什么可审查能力？**

今天的目标产物是：一个最小 ADK Agent 目录：agent、tools、evals、README、runbook 分开。

## 学习流程图

![Day 02 学习流程图](/images/academy/google-advent-of-agents/season-2/day-02-adk-agents-gemini-pro/diagram.svg)

这张图把当天主题拆成五步：先确认系统问题，再画设计边界，接着留下 artifact，然后设计 eval，最后回到生产风险。Agent 工程最怕的是“概念懂了，但系统没有多任何可验证能力”。

## 三个关键概念

- `root_agent` 是行为入口，不是所有知识的仓库。
- 工具要窄、可测试、可观察。
- README 要说明运行、失败和权限边界。

这三个概念可以作为读文章、写代码和做复盘时的抓手。只要其中一个说不清，系统通常还停留在 demo 阶段。

## 工程设计方式

把 Day 2 放进真实项目时，建议用下面这张小表约束设计：
...
真实项目里，第一个 ADK Agent 不应该叫 `demo.py`，而应该像一个小服务：有 `agent/`、`tools/`、`evals/`、`runbook.md` 和最小权限说明。这样下周换人接手时，他不需要读完整聊天记录，也能知道这个 Agent 能做什么、不能做什么、怎么验证。
...
- **业务触发：** 团队第一次把 ADK 放进真实 repo，需要避免 demo 代码直接进入长期维护区。
- **Agent 边界：** Agent 入口、工具、评估和运行手册分开；`root_agent` 只负责编排，不承载所有知识。
- **验收证据：** 新人能根据 README 跑通 smoke eval，并说清每个工具的权限边界。

## 最小 Lab

写一个只读工具型 Agent，让它调用一个受限工具并返回结构化答案。
...
Season 1 Day 2 / Day 3 / Day 6：YAML 入门、Gemini + ADK、开发环境。

Season 1 更像“从零到生产”的基础路线，Season 2 更像“把系统加厚”。读 Day 2 时，不需要回头逐日翻 Season 1，但应该知道它在补哪一层背景。
