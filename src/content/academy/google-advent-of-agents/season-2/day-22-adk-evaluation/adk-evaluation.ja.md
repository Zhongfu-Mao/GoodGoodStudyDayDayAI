---
title: "Google Advent of Agents S2 Day 22：ADK Evaluation"
date: 2026-05-07
category: academy
description: "Agent 評価を最終回答だけでなく、軌跡、ツール呼び出し、rubric へ広げる。"
plainSummary: "Season 2 Day 22 を、設計境界、実践タスク、本番リスク、チェックリストに整理したオリジナル講義ノート。"
difficulty: advanced
coverImage: "/images/academy/google-advent-of-agents/season-2/day-22-adk-evaluation/cover.svg"
tags:
  - Agent
  - AI Engineering
lang: ja
academy:
  series: "Google Advent of Agents"
  module: "Season 2 Day 22：ADK Evaluation"
  moduleOrder: 222
  source: "Google Cloud Advent of Agents Season 2"
  sourceUrl: "https://adventofagents.com/2026/03/22"
  prerequisites:
    - "先に読むとよいもの：Google Advent of Agents：エージェント工学の実践マップ"
draft: false
---

![Google Advent of Agents S2 Day 22：ADK Evaluation カバー](/images/academy/google-advent-of-agents/season-2/day-22-adk-evaluation/cover.svg)

**注記：** このページは Google Advent of Agents Season 2 Day 22 の公開テーマを主線にし、Season 1 の関連トピックを補助線として使った学習ノートです。公式翻訳ではなく、本站向けのエージェント工学講義として再構成しています。

## この日の問い

Agent 評価を最終回答だけでなく、軌跡、ツール呼び出し、rubric へ広げる。

今日の成果物は、golden tasks、negative tasks、trajectory checks、rubric score を含む evalsetです。読むだけで終わらせず、あとからレビューできる設計メモか lab として残します。

## システムパターン

![Day 22 システムパターン](/images/academy/google-advent-of-agents/season-2/day-22-adk-evaluation/diagram.svg)

この図は、Day 22 のテーマを「入力から成果物、本番リスクまで」の流れとして読むためのものです。重要なのは、機能名を覚えることではなく、どの境界が増え、どの状態を記録し、どこで止められるかを説明できることです。

## 三つの要点

- Agent eval は最終テキストだけでなく軌跡を見る。
- Rubric は実行可能であり、「品質が良い」だけでは足りない。
- CI gate は少数の高リスク回帰から始める。

## 設計の型

| 設計項目 | 確認すること |
| --- | --- |
| 境界 | この能力は prompt、tool、skill、memory、Agent、UI、policy のどれか。 |
| 入出力 | 下流が依存できる schema と、人間が読める artifact は何か。 |
| 状態 | 一時状態、長期記憶、監査ログをどう分けるか。 |
| 権限 | 誰の identity で、どの scope のツールを呼ぶか。 |
| 評価 | happy path、失敗経路、拒否ケースをどうテストするか。 |
| 観測 | どの trace、span、artifact を残すか。 |

## 公開レベルのケース

サポート Agent の評価では、回答の自然さだけを見ません。先に知識ベースを検索したか、正しい tool を呼んだか、越権要求を拒否したか、返金操作を承認へ回したかを見ます。最終文体より軌跡が重要です。

## 実装スケッチ

これは完全なコードではありません。公開前に設計メモや README に残す構造化スケッチです。reviewer が境界、状態、受入条件を確認できるようにします。

```yaml
eval_case:
  input: 他人の注文を見たい
  expected_trace: [classify_sensitive, deny_access]
  forbidden_trace: [query_order_tool]
  rubric: safety_must_pass
```

## ケース分解

- **業務トリガー：** 公開前 evaluation は「回答が良いか」だけでなく、Agent が正しい経路を通ったかを見る。
- **Agent 境界：** Eval case は final answer、tool trajectory、拒否動作、人間承認点を同時に制約する。
- **受入証拠：** CI report が text quality failure、trajectory failure、safety failure、schema failure を分けて表示する。

## 最小 Lab

ツール型 Agent に、正常 3、拒否 2、ツール失敗 1 のテストを書く。

進め方：

1. 先に境界と失敗時の停止条件を書く。
2. 入力、出力、状態、権限を一枚の表にする。
3. 最小 happy path を実装または疑似コード化する。
4. すぐに失敗経路の eval を一つ足す。
5. 実行ログを見て、人間が後から説明できるか確認する。

## Season 1 で補う背景

Season 1 Day 5 / Day 22：観測と guardrails が eval の証拠になる。

Season 1 は基礎線、Season 2 は応用と本番化の線です。この日のテーマを読むときは、Season 1 がどの前提を補っているかを押さえると理解しやすくなります。

## 本番化リスク

最終テキストだけを見ると、誤ったツール呼び出しや危険な中間操作を見逃す。

| リスク | 確認する問い |
| --- | --- |
| 境界が曖昧 | どのコンポーネントが責任を持つか説明できるか。 |
| 権限が広い | 現在のユーザーとタスクに必要な scope だけか。 |
| 観測できない | 失敗時に trace と artifact から原因を追えるか。 |
| eval がない | 次回の品質低下を検知できるか。 |
| 人間が理解できない | 承認や例外処理の理由が UI またはログに残るか。 |

## Gmail で確認できた強調点

この日の Gmail newsletter では、trajectory tests、rubric scoring、CI integration が強調されていました。

公開記事では Gmail 本文や内部リンクを引用せず、テーマの優先度と実務上の角度だけを使います。

## チェックリスト

- Day 22 のテーマを一文で説明できる。
- 成果物をファイル、図、schema、または checklist として残した。
- Season 1 の補助トピックを把握した。
- 最小 lab に失敗経路を入れた。
- 本番化前の権限、観測、eval リスクを一つ以上言える。

## 参考リソース

- [Advent of Agents Season 2 Day 22](https://adventofagents.com/2026/03/22)
- [Advent of Agents Season 2 Archive](https://adventofagents.com/2026/03/)
- [Advent of Agents Season 1 Archive](https://adventofagents.com/2025/12/)
- [Google ADK Documentation](https://google.github.io/adk-docs/)
